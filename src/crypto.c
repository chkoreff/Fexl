...
static int crypto_stream_salsa20_xor(u8 *c, const u8 *m, u64 b, const u8 *n,
	const u8 *k)
	{
	u8 z[16],x[64];
	u32 u,i;
...
static int crypto_stream_xor(u8 *c, const u8 *m, u64 d, const u8 *n,
	const u8 *k)
	{
	u8 s[32];
	crypto_core_hsalsa20(s,n,k,sigma);
...
int crypto_box_public(u8 *pk, const u8 *sk)
	{
	return crypto_scalarmult_base(pk,sk);
	}

int crypto_box_prepare(u8 *k, const u8 *pk, const u8 *sk)
	{
	u8 s[32];
...
int crypto_box_seal(u8 *c, const u8 *m, u64 d, const u8 *n, const u8 *k)
	{
	int i;
	if (d < 32) return -1;
...
int crypto_box_open(u8 *m, const u8 *c, u64 d, const u8 *n, const u8 *k)
	{
	int i;
	u8 x[32];
	if (d < 32) return -1;
...
int crypto_sign_public(u8 *pk, const u8 *sk)
	{
	u8 d[64];
	gf p[4];
...
int crypto_sign_seal(u8 *sm, const u8 *m, u64 n, const u8 *pk, const u8 *sk)
	{
	u8 d[64],h[64],r[64];
	i64 i,j,x[64];
	gf p[4];
...
int crypto_sign_open(u8 *m, const u8 *sm, u64 n, const u8 *pk)
	{
	int i;
	u8 t[32],h[64];
	gf p[4],q[4];
