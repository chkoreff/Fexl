...
//
// (Actually, UTF-8 continues to represent up to 31 bits with up to 6 bytes,
// but it is generally expected that the one million code points of the 20 bits
// offered by UTF-16 and 4-byte UTF-8 will suffice to cover all characters and
// that we will never get to see any Unicode character definitions beyond
// that.)
